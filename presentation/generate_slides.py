...
add_text_box(slide, 1, 1.5, 11, 1.2, "MCP Language Tools", font_size=54, bold=True, color=TEXT_WHITE, alignment=PP_ALIGN.CENTER)
...
add_text_box(slide, 7.3, 2.3, 5, 0.5, "MCP Language Tools: Best of both", font_size=17, bold=True, color=TEXT_YELLOW)
...
add_text_box(slide, 5.0, 2.9, 3.2, 0.5, "MCP Language Tools", font_size=15, bold=True, color=ACCENT, alignment=PP_ALIGN.CENTER)
...
add_text_box(slide, 1.2, 5.1, 10, 0.4, "How it works: MCP Language Tools manages inter-LSP communication", font_size=16, bold=True, color=ACCENT)

# MCP LT container
add_card(slide, 1.2, 5.6, 11, 1.5, RGBColor(0x22, 0x33, 0x55))
add_text_box(slide, 1.4, 5.6, 10, 0.3, "MCP Language Tools (orchestrator)", font_size=11, color=TEXT_GRAY)
...
slide = prs.slides.add_slide(prs.slide_layouts[6])
set_slide_bg(slide)
slide_title(slide, "Demo: Java Tools in Action", "Real examples of Claude Code using MCP Language Tools")
...
config_code = '''{
  "mcpServers": {
    "mcp-languagetools": { "url": "http://localhost:7654/mcp" }
  }
}'''
...
add_text_box(slide, 1, 3.8, 11, 0.8, "MCP Language Tools", font_size=28, color=ACCENT, bold=True, alignment=PP_ALIGN.CENTER)

add_bullet_list_linked(slide, 3, 5.0, 7, 0.5, [
    ("github.com/angelozerr/mcp-languagetools", "https://github.com/angelozerr/mcp-languagetools", ""),
...
# Save
output_path = os.path.join(SCRIPT_DIR, "MCP_LanguageTools.pptx")
