...
```json
{
  ...
  "command": {
    "windows": "${vscodeExtension:jetbrains.intellij-server}/server/bin/intellij-server.exe --stdio --system-path \"${workspaceStorageDir}\" --eula YOUR_EULA_KEY",
    "default": "${vscodeExtension:jetbrains.intellij-server}/server/bin/intellij-server --stdio --system-path \"${workspaceStorageDir}\" --eula YOUR_EULA_KEY"
  }
}
```
